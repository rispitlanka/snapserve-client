...
export default function SignInForm() {
  ...
  const handleSignIn = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const matchedUser = MOCK_USERS.find(
      (user) =>
        user.restaurantId.toLowerCase() === restaurantId.trim().toLowerCase() &&
        user.name.toLowerCase() === name.trim().toLowerCase() &&
        user.password === password
    );

    let finalUser: DynamicAdminUser | (typeof MOCK_USERS)[number] | null =
      matchedUser || null;
    ...
    if (!finalUser) {
      try {
        ...
        if (storedAdmins) {
          const parsed = JSON.parse(storedAdmins) as DynamicAdminUser[];
          finalUser =
            parsed.find(
              (user) =>
                user.restaurantId.toLowerCase() ===
    ...
    localStorage.setItem(MOCK_AUTH_STORAGE_KEY, JSON.stringify(finalUser));
    router.push(ROLE_DASHBOARD_ROUTE[finalUser.role]);
  };
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-800">
                  <p className="mb-2 font-medium text-gray-700 dark:text-gray-300">
                    Mock users (role-based)
                  </p>
                  <div className="space-y-1 text-gray-600 dark:text-gray-400">
                    <p>
                      Superadmin: 0RW0SH / Santhosh / test@123
                    </p>
                    <p>Admin: AD1001 / AdminUser / admin@123</p>
                    <p>Cashier: CS2001 / CashierUser / cashier@123</p>
                    <p>Cashier selects register after login.</p>
                  </div>
                </div>
              </div>
            </form>
